...
class PostActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FirebaseDatabase.getInstance().getReference("/Posts")
            .orderByChild("wrtieTime").addChildEventListener(object : ChildEventListener {
                //글이 추가된 경우
                override fun onChildAdded(snapshot: DataSnapshot, prevChildKey: String?) {
                    snapshot.let { snapshot ->
                        //snapshop의 데이터를 Post 객체로 가져옴
                        ...
                        post?.let {
                            ...
                            if (prevChildKey == null) {
                                ...
                                posts.add(it)
                                //RecyclerViewdml adapter에 글이 추가된 것을 알림
                ...
                override fun onChildChanged(snapshot: DataSnapshot, prevChildKey: String?) {
                    snapshot?.let{snapshot->
                        //snapshop의 데이터를 Post객체로 가져옴
                        val post=snapshot.getValue(Post::class.java)
                        post?.let{post->
                            //글이 변경된 경우 글의앞의 데이터 인덱스에 데이터를 변경
                            val prevIndex=posts.map{it.postId}.indexOf(prevChildKey)
                ...
                override fun onChildMoved(snapshot: DataSnapshot, prevChildKey: String?) {
                    snapshot?.let {
                        ...
                        post?.let { post ->
                            ...
                            //prevChildKey가 없는 경우 맨 마지막으로 이동
                            if (prevChildKey == null) {
                                posts.add(post)
                                recyclerView.adapter?.notifyItemChanged(posts.size - 1)
                            } else {
                                //prevChildeKey 다음 글로 추가
                                val prevIndex = posts.map { it.postId }.indexOf(prevChildKey)
                                posts.add(prevIndex + 1, post)
                                recyclerView.adapter?.notifyItemChanged(prevIndex + 1)
                            }
    ...
    inner class MyAdapter : RecyclerView.Adapter<MyViewHodler>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHodler {
            return MyViewHodler(
                LayoutInflater.from(this@PostActivity).inflate(
                    R.layout.card_post,
                    parent, false
                )
            )
        ...
        override fun onBindViewHolder(holder: MyViewHodler, position: Int) {
            ...
            //카드가 클릭되는 경우 DetailActivit를 실행
            holder.itemView.setOnClickListener {
                //상세화면을 호출할 Intent를 생성
                val intent=Intent(this@PostActivity, DetailActivity::class.java)
                //선택된 카드의 ID 정보를 intent에 추가한다
