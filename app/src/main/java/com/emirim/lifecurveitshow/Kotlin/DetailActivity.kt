        ...
        FirebaseDatabase.getInstance().getReference("/Comments/$postId").addChildEventListener(object
            ...
            override fun onChildMoved(snapshot: DataSnapshot, prevChildKey: String?) {
                if (snapshot != null) {
                    ...
                    comment?.let {
                        // 기존의 인덱스를 구한다
                        val existIndex = commentList.map { it.commetId }.indexOf(it.commetId)
                        // 기존에 데이터를 지운다.
                        commentList.removeAt(existIndex)
                        // prevChildKey 다음 글로 추가
                        val prevIndex = commentList.map { it.commetId }.indexOf(prevChildKey)
            ...
            override fun onChildRemoved(snapshot: DataSnapshot) {
                snapshot?.let {
                    ...
                    comment?.let { comment ->
                        ...
                        val existIndex = commentList.map { it.commetId}.indexOf(comment.commetId)
                        commentList.removeAt(existIndex)
